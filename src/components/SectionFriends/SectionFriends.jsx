...
export default function SectionFriends({ useNavigation, usePagination, customClass }) {
  ...
  const updateSlidesNavigation = () => {
    ...
    if (window.innerWidth < 768) {
      ...
      cards = splitArray(pets, 12);
    } else {
      cards = splitArray(pets, 8);
    }
  ...
  const updateSlidesPagination = () => {
    ...
    if (window.innerWidth < 768) {
      ...
      cards = splitArray(pets, 3);
    }

    setFinalCards(cards);
  }
  const updateSlides = (useNavigation) ? updateSlidesNavigation : updateSlidesPagination 
  useEffect(() => {
    updateSlides();
